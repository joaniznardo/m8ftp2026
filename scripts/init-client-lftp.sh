#!/bin/bash
# ─── init-client-lftp.sh ─────────────────────────────────────────────────────
# Script d'inicialització per al node client-lftp (Etapa 5).
# Executat per containerlab via exec: un cop eth1 existeix.
set -e

NODE_IP="${NODE_IP:-10.50.0.11}"
GW_IP="${GW_IP:-10.50.0.1}"
DNS_IP="${DNS_IP:-10.50.0.53}"
IFACE="${LAN_IFACE:-eth1}"

echo "[client-lftp] Configurant interfície: $IFACE → $NODE_IP/24"
...
echo "[client-lftp] Xarxa configurada. IP: $NODE_IP, GW: $GW_IP, DNS: $DNS_IP"

# Crear directori de treball per a ftpuser
